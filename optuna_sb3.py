...
def sample_ppo_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    n_steps = 2 ** trial.suggest_int("exponent_n_steps", 9, 12)
    vf_coef = trial.suggest_float("vf_coef", 0.45, 0.9, log=True)
    ent_coef = trial.suggest_float("ent_coef", 0, 0.005)
...
if __name__ == "__main__":
    ...
    storage_url = "sqlite:///ppo_source_study.db"

    study = optuna.create_study(sampler=sampler, pruner=pruner, direction="maximize", storage=storage_url, load_if_exists=True)
    try:
